...
Adapted for SmartPick-AI on 2026-09-18 from the reviewed SearchRank-AI Phase 7 snapshot
`9ee7afe`. SmartPick-AI retains general product-search branding and the first validated
smartphone catalogue. Internal package names and benchmark identifiers retain their source names.

## SmartPick-AI validation — 2026-09-18

The isolated copy passed **315 tests, with 6 optional integration tests skipped, in 11.02 seconds**.
Ruff lint, formatting for 58 files, dependency consistency, and Docker Compose configuration
validation passed. The six skips cover four live HTTP cases, the live provider, and PostgreSQL.
One Starlette/AnyIO test-client deprecation warning was recorded.

All 16 scripted agent scenarios passed with expected statuses, routes, tool selection, and retries.
The four applicable constrained cases and eight expected answered citation cases passed.
The unsupported-claim rate was zero in this controlled set; average tool calls were 2.1875.
This measures deterministic behavior with scripted model decisions, not live-model accuracy.

The 20-case retrieval rerun used the existing 3,062-product catalogue and aligned BM25/semantic
artifacts read-only, the pinned MiniLM model from local cache, CPU execution, and offline mode.
All five configurations reproduced the table in the historical retrieval section below.
The selected hybrid alpha remained 0.25: Recall@10 0.925000, MRR@10 0.950000,
NDCG@10 0.928558, and constraint satisfaction 1.000000.
Catalogue SHA-256: `c3428dd04e5d02c6a66ce00f5961f6b1f6ba9dcc30d86a37ffde84523619c7ea`.
Indexes were reused, not rebuilt; no new build-time or API-latency figure is claimed.

An initial suite run had 57 setup errors because the selected test temporary directory's parent
did not exist. Creating the local artifacts directory resolved those errors; no application code
or assertions were changed to obtain the passing result.

Compose uses `name: smartpick-ai`, separating named volumes from the original project.
No containers were started, no live database writes or paid LLM calls were made, and no new
image-build or live container verification result is claimed for this publication. Port 8000/8501
defaults still require free host ports. Generated evaluation reports remain local and ignored.

## Historical source report

The remaining report preserves the source project's actual September 11–12 measurements and
limitations. Its live Docker results apply to the specifically identified earlier source revision,
not to a new SmartPick-AI deployment. Reproduction commands apply to this repository, using fresh
output paths and locally configured dependencies.

Phase 7 broadens the source evidence without turning small, controlled benchmarks into
